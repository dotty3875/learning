...
#very confusing! basically should just be 1 + 2 + 3 + 4 + 5 + 6 + 7 until limit reached then prints the sum total to reach/surpass limit
...
while num < limit:  
    track += 1 #important to not change track other than 1 because it keeps the stable 1 + 2 + 3
    num += track  #if track is changed or num changed num then it doubles itself and adds +1 which is not the goal
