...
def first_word(sentence):
    first = sentence[0:sentence.find(" ")] # can also be sentence.split()[0] but didnt go this route at first because MOOC has not touched on split() afaik, just iteration
    return first
...
def last_word(sentence):
    track = -1
    for word in sentence.split(): # scalable way to find final word of a sentence 
        track += 1 
    last = sentence.split()[track]
    return last
